...
export async function POST(request: NextRequest) {
  const { userId } = await auth();
    ...
    if (conflictingAmmo > 0) {
      return NextResponse.json({ error: "Ammunition already exists" }, { status: 400 });
    }
  
    const newAmmo = await prisma.ammunition.create({
      data: {
        ...data,
        slug,
      },
    ...
    return NextResponse.json(newAmmo);
}